...
"""
Adapted from https://github.com/filippoferrari/CX_Path_Integration/blob/master/cx_spiking/optimisation/metric.py
Implemented by Ferrari F., (2020).
"""
...
def compute_gamma_factor_for_lists(model_spike_times, target_spike_times, time,
                                   dt_=defaultclock.dt, delta=1 * ms, rate_correction=True):
    all_gf = []
    for model, target in zip(model_spike_times, target_spike_times):
        gf = get_gamma_factor(model, target, delta, time, dt_, rate_correction=rate_correction)
        all_gf.append(gf)
